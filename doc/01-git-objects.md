...
이상, git 내부에 저장하는 오브젝트들을 간단히 살펴보고, 그 중 blob이 저장되는 형태에 대해 알아봤습니다. 어떠신가요? 왠지 git의 깊숙한 내부를 멋지게 파악한 것 같은 뿌듯함이 느껴지지 않나요? 다음 편에는 나머지 `tree`와 `commit` 오브젝트도 알아보도록 할게요. 그리고 나서, 물론 레퍼런스에 대해서도 알아봐야겠죠.
